...
abstract class GuidanceReadOnlyActionBase extends AiAssistantActionBase {
  ...
  public function listUsageInstructions(): array {
    return [
      ...
      'For Lesson 2 evaluation questions, use result labels: Complete, Partially complete, or Cannot confirm. Confirm whether CCC policy context covers brand voice, editorial standards, accessibility or governance, editor-facing scope, and draft-assistance-only boundaries. For editor-use evaluation, focus on current content/draft state, visible draft text, policy context availability, and the current user role boundary. Say Complete only when Drupal evidence or the learner statement confirms policy-guided editorial review or edits happened and the content remains draft/unpublished. If the user only asked for suggestions, say Partially complete and ask them to manually review or apply an appropriate edit, then verify the draft remains unpublished. If CCC source evidence is missing, say Cannot confirm and ask the user to open the saved CCC context item or listing.',
  ...
  private function sourceProvenance(GuidanceSource $source): string {
    if ($source->type === 'help_topic' && preg_match('/^Lesson\s+\d+:/', $source->title) === 1) {
      return 'Learn Drupal AI lesson rendered as a Drupal Help Topic';
    }
